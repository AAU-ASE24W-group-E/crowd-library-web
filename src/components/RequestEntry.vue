<template>
  <div @click="toggleDropdown"
    ...
    <div class="flex flex-row w-full">
      <img
        :src="`${config.OPENLIBRARY_COVER_URL}${props.request.book.data.coverId}.jpg`"
        alt="Cover"
        class="object-contain decoration-0 mr-3 w-[80px] max-sm:hidden"
      />
      <div class="flex flex-col flex-grow">
        <span class="text-xl font-semibold dark:text-title-dark-mode-text">Request by {{props.request.user.data.username}}: {{ props.request.book.data.title }}</span>
        <div class="flex flex-row max-sm:flex-col max-sm:mt-2 sm:space-x-8 max-sm:space-y-2">
          <div class="flex flex-col">
            <span class="tw-book-entry-info-title">Publisher: <span class="tw-book-entry-info-value">{{ props.request.book.data.publisher }}</span></span>
            <span class="tw-book-entry-info-title">Format: <span class="tw-book-entry-info-value">{{ props.request.book.data.format }}</span></span>
            <span class="tw-book-entry-info-title">Language: <span class="tw-book-entry-info-value">
                {{ props.request.book.data.languages && props.request.book.data.languages.length > 0 ? `${props.request.book.data.languages[0].toUpperCase()}` : 'No language found' }}</span></span>
          </div>
          <div class="flex flex-col">
            <span class="tw-book-entry-info-title">ISBN: <span class="tw-book-entry-info-value">{{ props.request.book.data.isbn }}</span></span>
            <span class="tw-book-entry-info-title">Meeting Date: <span class="tw-book-entry-info-value">{{ props.request.lending.lendingMeeting?.meetingTime ? formatDate(props.request.lending.lendingMeeting.meetingTime) : 'No meeting' }}</span></span>
            <span class="tw-book-entry-info-title">Location: <span class="tw-book-entry-info-value">{{ props.request.lending.lendingMeeting?.meetingLocation || 'No meeting' }}</span></span>
          </div>
          <div class="flex flex-col">
    ...
    <div v-bind:class="{
      ...
      <div
        ...
        <button v-if="incoming && props.request.lending.status === LendingStatus.READER_CREATED_REQUEST" @click="openPopup" v-show="dropdownOpen" id="suggestMeetingBtn" class="btn-primary btn-green rounded-2xl">Suggest Meeting</button>
        <button v-if="!incoming && props.request.lending.status === LendingStatus.OWNER_SUGGESTED_MEETING" @click="confirmMeeting" v-show="dropdownOpen" id="confirmMeetingBtn" class="btn-primary btn-green rounded-2xl">Confirm Meeting</button>
        <button v-if="incoming && props.request.lending.status === LendingStatus.READER_ACCEPTED_MEETING" @click="confirmTransferOwner" v-show="dropdownOpen" id="confirmTransferOwnerBtn" class="btn-primary btn-green rounded-2xl">Confirm Transfer</button>
        <button v-if="!incoming && props.request.lending.status === LendingStatus.OWNER_CONFIRMED_TRANSFER" @click="confirmTransferReader" v-show="dropdownOpen" id="confirmTransferReaderBtn" class="btn-primary btn-green rounded-2xl">Confirm Transfer</button>
      </div>
    </div>
  </div>
...
const declineIncomingLending = async () => {
  try {
    await lendingService.declineLendingRequest(props.request.lending.id);
...
const cancelOutgoingLending = async () => {
  try {
    await lendingService.cancelLendingRequest(props.request.lending.id);
...
const confirmMeeting = async () => {
  try {
    await lendingService.updateLendingStatus(props.request.lending.id, LendingStatus.READER_ACCEPTED_MEETING);
...
const confirmTransferOwner = async () => {
  try {
    await lendingService.updateLendingStatus(props.request.lending.id, LendingStatus.OWNER_CONFIRMED_TRANSFER);
...
const confirmTransferReader = async () => {
  try {
    await lendingService.updateLendingStatus(props.request.lending.id, LendingStatus.READER_CONFIRMED_TRANSFER);
