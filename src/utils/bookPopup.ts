      ...
      <div class="text-center p-2 font-sans rounded-lg bg-white">
        ...
        <div class="bg-gray-50 p-3 mt-2 rounded-md">
         <p class="mt-2 text-sm font-semibold ${
           book_properties.status === 'AVAILABLE' ? 'text-green-600' : 'text-red-600'
         }">${book_properties.status === 'AVAILABLE' ? 'Available' : 'Unavailable'} </p>
          <p class="text-sm text-gray-700"><span class="font-semibold">Owner:</span> ${book_properties.owner.name} ${starIconHTMLString} ${mockRatingValue.value} (${mockRatingNumber.value})</p>
